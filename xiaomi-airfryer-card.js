 ...
 * 
 * Instalare:
 *   1. Copiază fișierul în /config/www/xiaomi-airfryer-card.js
 *   2. În HA → Settings → Dashboards → Resources → Add:
 *      URL: /local/xiaomi-airfryer-card.js  |  Type: JavaScript module
 *   3. Folosește în Lovelace:
 * 
...
class XiaomiAirFryerCard extends HTMLElement {
  ...
  // ── Lovelace lifecycle ──────────────────────────────────────
  setConfig(config) {
    if (!config.device) throw new Error('Specifică "device: <prefix_entitate>"');
    this._config = config;
  ...
  // ── Entity helpers ──────────────────────────────────────────
  _eid(suffix) { return `${this._device}_${suffix}`; }
  _state(suffix) { return this._hass?.states[this._eid(suffix)]?.state ?? 'unavailable'; }
  _attr(suffix, attr) { return this._hass?.states[this._eid(suffix)]?.attributes?.[attr]; }

  _callService(domain, service, entityId, data = {}) {
    this._hass.callService(domain, service, { entity_id: entityId, ...data });
  }
  _pressButton(suffix) { this._callService('button', 'press', this._eid(suffix)); }
  _setNumber(suffix, value) { this._callService('number', 'set_value', this._eid(suffix), { value }); }
  _selectOption(suffix, option) { this._callService('select', 'select_option', this._eid(suffix), { option }); }
  _toggleSwitch(suffix) { this._callService('homeassistant', 'toggle', this._eid(suffix)); }
  ...
  // ── Update from hass state ──────────────────────────────────
  _update() {
    if (!this._hass || !this._built) return;

    const status   = this._state('air_fryer');
    const leftTime = this._state('left_time');
    const temp     = this._state('target_temperature');
    const time     = this._state('target_time');
    const mode     = this._state('mode');
    const texture  = this._state('texture');
    const measure  = this._state('target_cooking_measure');
    const turnPot  = this._state('turn_pot');
    const preheat  = this._state('preheat');
    const keepWarm = this._state('auto_keep_warm');
    ...
    if (leftTime !== 'unavailable') {
      this._q('#lcdTime').textContent = left + ' min';
      this._q('#statLeft').textContent = left;
      this._q('#progLabel').textContent = `⏱ ${left} min rămași`;
    ...
    // toggles
    this._setToggle('#togglePreheat', preheat === 'on');
    this._setToggle('#toggleKeepWarm', keepWarm === 'on');
    this._setToggle('#toggleCurrWarm', currWarm === 'on');
    this._setToggle('#toggleTurnCfg', turnCfg === 'on');
  }
  ...
  _setToggle(sel, on) {
    this._q(sel)?.classList.toggle('on', on);
  }
  ...
  _attachEvents() {
    ...
    // temp +/-
    this._q('#tempMinus').addEventListener('click', () => {
      const v = parseFloat(this._state('target_temperature')) || 180;
      this._setNumber('target_temperature', Math.max(40, v - 5));
    });
    this._q('#tempPlus').addEventListener('click', () => {
      const v = parseFloat(this._state('target_temperature')) || 180;
      this._setNumber('target_temperature', Math.min(230, v + 5));
    });

    // time +/-
    this._q('#timeMinus').addEventListener('click', () => {
      const v = parseFloat(this._state('target_time')) || 20;
      this._setNumber('target_time', Math.max(1, v - 5));
    });
    ...
    this._q('#toggleKeepWarm').addEventListener('click', () => this._toggleSwitch('auto_keep_warm'));
    this._q('#toggleCurrWarm').addEventListener('click', () => this._toggleSwitch('current_keep_warm'));
  ...
  _openPanel() {
    ...
    this._q('#configPanel').classList.add('open');
    this._q('#overlay').classList.add('visible');
  }
  _closePanel() {
    this._panelOpen = false;
    this._q('#configPanel').classList.remove('open');
    this._q('#overlay').classList.remove('visible');
  }
  ...
  _buildMesh() {
    ...
    setInterval(() => {
      this.shadowRoot.querySelectorAll('.af-hole').forEach(c =>
        c.classList.toggle('hot', Math.random() > 0.52)
      );
    }, 500);
  }
  ...
  _buildHeatWaves() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      w.className = 'af-hw';
      w.style.cssText = `left:${6 + i * 13}px;animation-delay:${Math.random() * 2}s;animation-duration:${1.4 + Math.random() * 0.8}s;--dx:${(Math.random() - 0.5) * 8}px`;
      heat.appendChild(w);
    }
  }

  // ── Optimistic local timer (overridden by hass updates) ─────
  _startTimer() {
    ...
    <div class="card-wrap">
      ...
      <div class="config-panel" id="configPanel">
        <div class="panel-handle" id="panelHandle">
          ...
          <div class="panel-handle-sub">${this._device} · MAF10A</div>
        </div>
        ...
        <div class="panel-scroll">
          ...
          <div class="num-row">
            <div class="num-control">
              ...
              <div class="num-mid">
                <div class="num-value" id="tempVal">— <small>°C</small></div>
                <div class="num-btns">
                  <button class="num-btn" id="tempMinus">−</button>
                  <button class="num-btn" id="tempPlus">+</button>
                </div>
              </div>
            </div>
            <div class="num-control">
              ...
              <div class="num-mid">
                <div class="num-value" id="timeVal">— <small>min</small></div>
                <div class="num-btns">
                  <button class="num-btn" id="timeMinus">−</button>
                  <button class="num-btn" id="timePlus">+</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="toggles-grid">
            ...
            <div class="toggle-card" id="toggleKeepWarm">
              <div class="toggle-left"><span>♨️</span><span class="toggle-name">Auto Warm</span></div>
              <div class="toggle-pill"><div class="toggle-thumb"></div></div>
            </div>
            <div class="toggle-card" id="toggleCurrWarm">
  ...
  _css() { return `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .ha-card{background:#181c27;border:1px solid #2a2f45;border-radius:24px;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,0.6)}
    ...
    .progress-fill{height:100%;border-radius:4px;position:relative;background:linear-gradient(90deg,#dc2626,#f97316);transition:width .5s ease}
    ...
    .btn-resume{color:#60a5fa}
    .btn-stop{color:#f87171}

    .panel-overlay{position:fixed;inset:0;background:rgba(5,6,12,.7);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);z-index:100;opacity:0;pointer-events:none;transition:opacity .3s ease}
    ...
    .num-row{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:4px}
    ...
    .num-btn{width:26px;height:26px;border-radius:8px;border:1px solid #2a2f45;background:#252a3d;color:#e8eaf6;font-size:13px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;font-family:'Syne',sans-serif}
    .num-btn:hover{border-color:#f97316;color:#f97316}

    .chips-wrap{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:4px}
    .chip{padding:5px 10px;border-radius:10px;border:1px solid #2a2f45;background:#1e2335;color:#6b7280;font-size:10px;font-weight:600;font-family:'Syne',sans-serif;cursor:pointer;transition:all .2s;white-space:nowrap}
    .chip.active{background:rgba(249,115,22,.15);border-color:rgba(249,115,22,.5);color:#f97316}

    .sel-row{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:4px}
    ...
    .toggles-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:4px}
